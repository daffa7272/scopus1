    ...
    # ---------------------------------------------------------
        ...
        if not geo_col_target:
            ...
            with st.spinner("Memisahkan korpus data Global dan Indonesia..."):
                ...
                mask_indo = data[geo_col_target].astype(str).str.contains(r'\bIndonesia\b', case=False, na=False)
                
                df_indo = data[mask_indo].copy()
                df_global = data[~mask_indo].copy()
            ...
            if df_indo.empty:
                ...
                tab_metrics, tab_collab, tab_topic, tab_journal, tab6, tab_ai_comp = st.tabs([
                    "📈 Pertumbuhan & Dampak", "🤝 Pemetaan Kolaborasi", "🎯 Analisis Topik (Venn)", "🏢 Kualitas Wadah Publikasi", "🇮🇩 AI Sintesis (Indo)", "🧠 AI Strategic Advisor"
                ])
